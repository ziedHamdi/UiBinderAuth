 ...
 * annotated with {@link fr.onevu.gwt.uibinder.client.UiHandler} so that the
 ...
 * {@link fr.onevu.gwt.uibinder.client.UiField} as long as it exists (annotated
 * with ui:field) in the template.
 */
class HandlerEvaluator {
	 ...
	 * and into fr.onevu.gwt.uibinder.rebind.model
	 */
	 ...
	 */
	protected void writeHandler(IndentedWriter writer, String uiOwner, String handlerVarName, JClassType handlerType, JClassType eventType, String boundMethod)
		...
		writer.newline();
		// Create the anonymous class extending the raw type to avoid errors under
		// the new JDT
		...
		// Use the event raw type to match the signature as we are using
		// implementing the raw type
		// interface.
	 ...
	 */
	void writeAddHandler(IndentedWriter writer, FieldManager fieldManager, String handlerVarName, String addHandlerMethodName, String objectName) {
	...
	private JMethod getAddHandlerMethodForObject(JClassType objectType, JClassType handlerType) throws UnableToCompleteException {
		...
		for (JMethod method : objectType.getInheritableMethods()) {
			...
			if (returnClassType != null && handlerRegistrationJClass.isAssignableFrom(returnClassType)) {
				 ...
				 * 
				 * handlerType => TableHandler<String> subjectHandler => Alt 1:
				 * TableHandler or Alt 2: TableHandler<T>
				 * 
				 * This is done as an alternative handler method to preserve the
				 * original logic.
				 */
				if (handlerType.isAssignableFrom(methodParam)) {
					// Alt 1: TableHandler<String> => TableHandler or TableHandler<?> =>
					// TableHandler<String>
	...
	private JClassType getHandlerForEvent(JClassType eventType) {
		...
		if (isParameterized == null) {
			logger.warn("The method 'getAssociatedType()' in '%s' does not return Type<? extends EventHandler>.", eventType.getName());
			return null;
		}
