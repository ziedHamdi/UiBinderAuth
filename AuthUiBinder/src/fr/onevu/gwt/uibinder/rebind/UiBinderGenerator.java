 ...
 * {@link fr.onevu.gwt.uibinder.client.UiBinder}.
 */
public class UiBinderGenerator extends Generator {
	...
	public String generate(TreeLogger logger, GeneratorContext genCtx, String fqInterfaceName) throws UnableToCompleteException {
		...
		if (printWriter != null) {
			generateOnce(interfaceType, implName, printWriter, logger, oracle, resourceOracle, genCtx.getPropertyOracle(), writers, designTime);
		}
		return packageName + "." + implName;
	}

	private Boolean extractConfigProperty(MortalLogger logger, PropertyOracle propertyOracle, String configProperty, boolean defaultValue) {
		List<String> values;
		try {
	...
	protected void generateOnce(JClassType interfaceType, String implName, PrintWriter binderPrintWriter, TreeLogger treeLogger, TypeOracle oracle,
			ResourceOracle resourceOracle, PropertyOracle propertyOracle, PrintWriterManager writerManager, DesignTimeUtils designTime)
		...
		FieldManager fieldManager = createFieldManager(oracle, logger, useLazyWidgetBuilders);

		UiBinderWriter uiBinderWriter = new UiBinderWriter(interfaceType, implName, templatePath, oracle, logger, fieldManager, messages, designTime, uiBinderCtx,
	...
	protected FieldManager createFieldManager(TypeOracle oracle, MortalLogger logger, boolean useLazyWidgetBuilders) {
		return new FieldManager(oracle, logger, useLazyWidgetBuilders);
	}

	private Document getW3cDoc(MortalLogger logger, DesignTimeUtils designTime, ResourceOracle resourceOracle, String templatePath)
			throws UnableToCompleteException {
	...
	private Boolean useLazyWidgetBuilders(MortalLogger logger, PropertyOracle propertyOracle) {
		...
		if (!gaveLazyBuildersWarning && !rtn) {
			logger.warn("Configuration property %s is false. Deprecated code generation is in play. " + "This property will soon become a no-op.",
	...
	private Boolean useSafeHtmlTemplates(MortalLogger logger, PropertyOracle propertyOracle) {
		...
		if (!gaveSafeHtmlWarning && !rtn) {
			logger.warn("Configuration property %s is false! UiBinder SafeHtml integration is off, "
					...
					+ "will soon become a no-op, and SafeHtml integration will always be on.", XSS_SAFE_CONFIG_PROPERTY);
