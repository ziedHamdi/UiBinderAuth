 ...
 * relevant to the code of the owner class. The fields in the binder class are
 * instead represented by an instance of
 * {@link fr.onevu.gwt.uibinder.rebind.FieldWriter}.
 */
public class OwnerField {
	 ...
	 * 
	 * @param field
	 *          the field of the owner class
	 * @param logger
	 * @param context
	 */
	public OwnerField(JField field, MortalLogger logger, UiBinderContext context) throws UnableToCompleteException {
		...
		if (fieldClassType == null) {
			logger.die("Type for field " + name + " is not a class: " + field.getType().getSimpleSourceName());
		}

		this.fieldType = OwnerFieldClass.getFieldClass(fieldClassType, logger, context);
	...
	/**
	 * Returns whether this field's value is provided by owner class. If it's not
	 * provided, then it's the binder's responsibility to assign it.
	 */
	...
	public String toString() {
		return String.format("%s.%s#%s", fieldType.getRawType().getPackage(), fieldType.getRawType().getName(), name);
	}
}
