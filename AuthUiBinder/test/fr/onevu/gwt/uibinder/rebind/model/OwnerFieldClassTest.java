...
import fr.onevu.gwt.uibinder.rebind.JClassTypeAdapter;
...
public class OwnerFieldClassTest extends TestCase {
	...
	public void testOwnerFieldClass() throws Exception {
		...
		OwnerFieldClass fieldClass = OwnerFieldClass.getFieldClass(htmlType, MortalLogger.NULL, uiBinderCtx);
		...
		assertSame(fieldClass, OwnerFieldClass.getFieldClass(htmlType, MortalLogger.NULL, uiBinderCtx));
	...
	public void testCheckBoxValueSetters() throws Exception {
		...
		OwnerFieldClass settersClass = OwnerFieldClass.getFieldClass(cbClassType, MortalLogger.NULL, uiBinderCtx);
	...
	public void testDisambiguateClassHierarchySettersBase() throws Exception {
		...
		OwnerFieldClass settersClass = OwnerFieldClass.getFieldClass(baseClassType, MortalLogger.NULL, uiBinderCtx);
	...
	public void testDisambiguateClassHierarchySettersDerived() throws Exception {
		...
		OwnerFieldClass settersClass = OwnerFieldClass.getFieldClass(derivedClass, MortalLogger.NULL, uiBinderCtx);
		...
		// base.value2(Integer) and derived.value2(int) is not ambiguous - must be
		// int
		assertNotNull(settersClass.getSetter("value2"));
		assertMethod(settersClass.getSetter("value2"), "setValue2", JPrimitiveType.INT);

		// base.value3 (int) and derived.value3(Integer) is not ambiguous - must be
		// int.
		...
		assertMethod(settersClass.getSetter("value6"), "setValue6", gwtTypeAdapter.adaptJavaClass(String.class));

		// base.value7(object) and base.value7(int) is not ambiguous - must be int.
		assertNotNull(settersClass.getSetter("value7"));
		assertMethod(settersClass.getSetter("value7"), "setValue7", JPrimitiveType.INT);
	}
	...
	public void testOwnerFieldClass_withUiChildren() throws Exception {
		...
		OwnerFieldClass parentClass = OwnerFieldClass.getFieldClass(parentType, MortalLogger.NULL, uiBinderCtx);
	...
	public void testOwnerFieldClass_withMultipleUiConstructors() {
		...
		try {
			OwnerFieldClass.getFieldClass(constructorsType, MortalLogger.NULL, uiBinderCtx);
			fail("Expected exception not thrown");
		} catch (UnableToCompleteException utce) {
			// Expected
		}
	 ...
	 */
	private void assertMethod(JMethod method, String methodName, JType... parameterTypes) {
		assertNotNull(method);
		assertEquals(methodName, method.getName());
		JParameter[] parameters = method.getParameters();
		assertEquals(parameterTypes.length, parameters.length);
		for (int i = 0; i < parameters.length; i++) {
			assertEquals("Parameter " + i + " of method " + methodName + " mismatch. Expected" + parameterTypes[i].getSimpleSourceName() + "; actual: "
					+ parameters[i].getType().getSimpleSourceName(), parameterTypes[i], parameters[i].getType());
		}
	}
}
